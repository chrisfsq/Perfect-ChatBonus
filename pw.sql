...
--
-- Host: localhost
-- Generation Time: 04-Abr-2024 às 15:25
...
-- Procedures
--
CREATE DEFINER=`admin`@`%` PROCEDURE `acquireuserpasswd`(in name1 VARCHAR(64), out uid1 INTEGER, out passwd1 VARCHAR(64))
BEGIN
    SELECT id, passwd INTO uid1, passwd1 FROM users WHERE name = name1;
END$$

CREATE DEFINER=`admin`@`%` PROCEDURE `addForbid`(in userid1 INTEGER, in type1 INTEGER, in forbid_time1 INTEGER, in reason1 BINARY(255), in gmroleid1 INTEGER)
BEGIN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `addGM`(in userid INTEGER, in zoneid INTEGER)
BEGIN
  DECLARE x INTEGER;
  START TRANSACTION;
    SET x = 0;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `adduserpoint`(in uid1 INTEGER, in aid1 INTEGER, in time1 INTEGER)
BEGIN
 DECLARE rowcount INTEGER;
 START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `addUserPriv`(in userid INTEGER, in zoneid INTEGER, in rid INTEGER)
BEGIN
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `changePasswd`(in name1 VARCHAR(64), in passwd1 VARCHAR(64))
BEGIN
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `changePasswd2`(in name1 VARCHAR(64), in passwd21 VARCHAR(64))
BEGIN
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `clearonlinerecords`(in zoneid1 INTEGER, in aid1 INTEGER)
BEGIN
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `deleteTimeoutForbid`(in userid1 INTEGER)
BEGIN
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `delUserPriv`(in userid1 INTEGER, in zoneid1 INTEGER, in rid1 INTEGER, in deltype1 INTEGER)
BEGIN
START TRANSACTION;
  IF deltype1 = 0 THEN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `enableiplimit`(in uid1 INTEGER, in enable1 CHAR(1))
BEGIN
  DECLARE rowcount INTEGER;
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `lockuser`(in uid1 INTEGER, in lockstatus1 CHAR(1))
BEGIN
  DECLARE rowcount INTEGER;
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `recordoffline`(in uid1 INTEGER, in aid1 INTEGER, inout zoneid1 INTEGER, inout zonelocalid1 INTEGER, inout overwrite1 INTEGER)
BEGIN
  DECLARE rowcount INTEGER;
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `recordonline`(in uid1 INTEGER, in aid1 INTEGER, inout zoneid1 INTEGER, inout zonelocalid1 INTEGER, inout overwrite INTEGER)
BEGIN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `remaintime`(in uid1 INTEGER, in aid1 INTEGER, out remain INTEGER, out freetimeleft INTEGER)
BEGIN
...
CREATE DEFINER=`admin`@`%` PROCEDURE `setiplimit`(in uid1 INTEGER, in ipaddr11 INTEGER, in ipmask11 VARCHAR(2), in ipaddr21 INTEGER, in ipmask21 VARCHAR(2), in ipaddr31 INTEGER, in ipmask31 VARCHAR(2), in enable1 CHAR(1))
BEGIN
  DECLARE rowcount INTEGER;
  START TRANSACTION;
...
CREATE DEFINER=`admin`@`%` PROCEDURE `usecash`(
  in userid1 INTEGER,
  in zoneid1 INTEGER,
  in sn1 INTEGER,
  in aid1 INTEGER,
...
--
-- Estrutura da tabela `usebonuslog`
--
...
--
-- Estrutura da tabela `usecashlog`
--
...
--
-- Estrutura da tabela `usecashnow`
--
...
(1024, '1', 'ZRK9Q9nKpuAsmQsKgmUtyg==', '', '', '1', '', '1', '', '', '', '', '', '', 0, NULL, '2024-03-13 15:34:30', '', ''),
(1040, '1234', '7SsfRoxfkV8/HPddcGi6rg==', '', '', '4124', '', '1241', '', '', '', '', '', '', 0, NULL, '2024-03-13 15:37:48', '', ''),
(1056, '12345', 'jPoigrF94KWYwBD18BCefQ==', '', '', '12345', '', '12345', '', '', '', '', '', '', 0, NULL, '2024-03-30 15:12:59', '', '');
...
ALTER TABLE `usecashnow`
  ...
  ADD KEY `IX_usecashnow_creatime` (`creatime`),
